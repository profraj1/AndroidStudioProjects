...
class ForgotPasswordActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnNext.setOnClickListener {
            if(etForgotPwdMobileNo.text.isEmpty() or (etForgotPwdMobileNo.text.toString().length< 10)){
                Toast.makeText(this@ForgotPasswordActivity, "Please Enter a valid Mobile Number", Toast.LENGTH_SHORT).show()
            }
            else if(etForgotPwdEmail.text.isEmpty()){
                Toast.makeText(this@ForgotPasswordActivity, "Please Enter your Email Address", Toast.LENGTH_SHORT).show()
            }
            else if(ConnectionManager().checkConnectivity(this@ForgotPasswordActivity)){
                ...
                Response.Listener {
                    try{
                        ...
                        Toast.makeText(this@ForgotPasswordActivity, "No data Found!!", Toast.LENGTH_SHORT).show()
                    }
                },
                Response.ErrorListener {
                    Toast.makeText(this@ForgotPasswordActivity, "No Response from the server", Toast.LENGTH_SHORT).show()
                }){
